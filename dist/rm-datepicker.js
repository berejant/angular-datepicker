        ...
        function (rmDatepickerConfig, $compile, $filter, $document, $timeout) {
            ...
            var link = function (scope, element, attrs, ngModel) {
                ...
                if (isInput) {
                    ...
                    var isYear = function(string) {
                        return string.length == 4 || (string.length == 2 && string >= 40);
                    };

                    ngModel.$parsers.push(function (inputString) {
                        if(!inputString) {
                            return null;
                        }
                        ...
                        var parts = inputString.trim().split(/[^0-9]+/);

                        if(parts.length != 3) {
                            return;
                        }

                        if(isYear(parts[0])) { // maybe reverse date such as "1990-01-30"
                            if(isYear(parts[2])) { // WTF ? date sush as "1990-01-1990" or "90-01-90" - not correctly
                                return;
                            }
                        ...
                        var date = new Date(year, month - 1, day, 3, 0, 1, 0);

                        if((!conf.min || conf.min < date) && (!conf.max || date < conf.max)) {
                            return date;
                        }
                    ...
                    ngModel.$validators.required = function () {
                        console.log($scope.val, getIsDefault());
